...
image_name="coilyco-flight-deck/bluesky-mcp"
...
if [ -z "${FORGEJO_EGRESS_PROXY:-}" ]; then
  echo "FORGEJO_EGRESS_PROXY is required for the Bluesky MCP dependency build." >&2
  exit 1
fi
...
case "${sha}" in
  *[!0-9a-f]*|"")
    echo "bluesky-mcp source sha is not a lowercase hexadecimal commit id." >&2
    exit 1
    ;;
esac
if [ "${#sha}" -ne 40 ]; then
  echo "bluesky-mcp source sha must be a full 40-character commit id." >&2
  exit 1
fi
